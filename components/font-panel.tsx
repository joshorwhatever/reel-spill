  ...
  async function loadFont(files: File[]) {
    for (const file of files) {
      ...
      try {
        ...
        handleAddFont(f)
        handleChange({ fontId: f.id, randomFont: false })
      } catch {
    ...
    >
      <div className="flex flex-col h-full min-h-0 gap-2">
        <div className="flex flex-col gap-1 shrink-0">
          ...
          {/* Invert, Lines, Random controls */}
          <div className="flex gap-1 mt-0.5">
            <button
              type="button"
              aria-pressed={Boolean(style?.invert)}
              onClick={() => handleChange({ invert: !style?.invert })}
              className={cn(
            ...
            <button
              type="button"
              aria-pressed={Boolean(style?.lines && style.lines > 0)}
              onClick={() => {
                const current = style?.lines || 0
                const next = current >= 4 ? 0 : current + 1
                handleChange({ lines: next })
              }}
              className={cn(
                'flex-1 border py-0.5 font-mono text-[10px] uppercase tracking-[0.14em] transition-colors cursor-pointer',
                style?.lines && style.lines > 0
                  ? 'border-cream/70 bg-cream/10 text-cream'
                  : 'border-border text-cream-dim hover:border-cream/40',
              )}
            >
              {style?.lines && style.lines > 0 ? `lines (${style.lines})` : 'lines'}
            </button>

            <button
              type="button"
              disabled={safeCustomFonts.length > 0}
              aria-pressed={Boolean(style?.randomFont)}
              onClick={() => handleChange({ randomFont: !style?.randomFont })}
              className={cn(
                'flex-1 border py-0.5 font-mono text-[10px] uppercase tracking-[0.14em] transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed',
                style?.randomFont
                  ? 'border-cream/70 bg-cream/10 text-cream'
                  : 'border-border text-cream-dim hover:border-cream/40',
              )}
              title={safeCustomFonts.length > 0 ? 'Disabled because custom font is uploaded' : undefined}
            >
              random
            </button>
          </div>
          ...
          <div className="flex gap-1 mt-0.5">
            <button
              type="button"
              aria-pressed={Boolean(style?.uppercase)}
              onClick={() => handleChange({ uppercase: !style?.uppercase })}
              className={cn(
                'flex-1 border py-0.5 font-mono text-[10px] uppercase tracking-[0.14em] transition-colors cursor-pointer',
                style?.uppercase
                  ? 'border-cream/70 bg-cream/10 text-cream'
            ...
            <button
              type="button"
              aria-pressed={Boolean(style?.shadow)}
              onClick={() => handleChange({ shadow: !style?.shadow })}
              className={cn(
                'flex-1 border py-0.5 font-mono text-[10px] uppercase tracking-[0.14em] transition-colors cursor-pointer',
                style?.shadow
                  ? 'border-cream/70 bg-cream/10 text-cream'
